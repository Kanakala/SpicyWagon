...
class EmailUserManager(BaseUserManager):
    ...
    def _create_user(self, password,
        ...
        user = self.model(is_staff=is_staff, is_active=is_active,
    ...
    def create_user(self, password=None, **extra_fields):
        ...
        return self._create_user(password, is_staff, False,
                                 **extra_fields)
    ...
    def create_superuser(self, password, **extra_fields):
        ...
        return self._create_user( password, True, True,
                                 **extra_fields)
...
class AbstractEmailUser(AbstractBaseUser, PermissionsMixin):
    ...
    username = models.CharField(_('phone number'), max_length=15,
                              unique=True, db_index=True)
    name = models.CharField(_('your name'), max_length=30,
                               db_index=True)
...
class EmailUser(AbstractEmailUser):
    ...
    def assign_group(self, email, phone, name, password, **extra_fields):
        user = self.model(email=email, username=username, name=name, phone=phone, is_staff=is_staff, is_active=is_active,
                          is_superuser=is_superuser, last_login=now,
                          date_joined=now, **extra_fields)
    ...
    def assign_group(self, email, password, name, username, **extra_fields):
        user = self.model(email=email, is_staff=is_staff, is_active=is_active,
                          is_superuser=is_superuser, last_login=now,
                          date_joined=now, username=username, name=name, Label=label, **extra_fields)
        if user.Label == 'Partner':
            g1 = Group.objects.get(name=Partner)
            g1.user_set.add(Partner_USER)
            user_group.save()
            user.save(using=self._db)
            return user
        elif user.Label == 'Customer':
            g2 = Group.objects.get(name=Customer)
            g2.user_set.add(Customer_USER)
            user_group.save()
            user.save(using=self._db)
            return user
